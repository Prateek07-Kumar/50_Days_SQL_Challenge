...
SELECT 
    ...
    --DATEDIFF(DAY, a1.purchase_date, a2.purchase_date) AS days_between
FROM amazon_transactions a1
INNER JOIN amazon_transactions a2
    ON a1.user_id = a2.user_id 
    AND a1.purchase_date < a2.purchase_date
    AND DATEDIFF(DAY, a1.purchase_date, a2.purchase_date) <= 7
	order by 1;
...
WITH Purchases AS (
	select 
		user_id,
		purchage_date,
		ROW_NUMBER() OVER(Partition by user_id order by purchage_date) as rn
...
INNER JOIN amazon_transactions a2
	...
	AND a2.rn = 2
    --AND a1.purchase_date < a2.purchase_date
 WHERE DATEDIFF(DAY, a1.purchase_date, a2.purchase_date) <= 7;
 */
...
SELECT 
    ...
    DATEDIFF(DAY, p1.purchase_date, p2.purchase_date) AS days_between
FROM Purchases p1
JOIN Purchases p2 
