...
(
	select 
		...
		rank() over(partition by month(invoicedate) order by sum(unitprice * quantity) desc) as rn
	from walmart_eu
...
Your Task
Find Customer of the month from each MONTH one customer who has spent the highest 
amount (price * quantity) as total amount may include multiple purchase
