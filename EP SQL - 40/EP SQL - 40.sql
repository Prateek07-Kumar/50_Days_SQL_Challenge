...
SELECT 
    ...
    COUNT(*) AS cnt_orders,
    ROUND(
        CAST(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM order_data) AS DECIMAL(5, 2)), 
        2
    ) AS order_percentage
FROM order_data
...
SELECT 
	case
		...
		when datepart(hour, order_time) >= 12 AND datepart(hour, order_time) < 17 then 'After_noon'
		when datepart(hour, order_time) >= 17 AND datepart(hour, order_time) < 20 then 'Evening'
	else 'night'
