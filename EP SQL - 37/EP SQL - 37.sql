...
INSERT INTO customers4 (customer_id, customer_name) VALUES
(1, 'John'),
(2, 'Alice'),
(3, 'Bob'),
(4, 'Charlie');
...
where month(spending_date) = 3  -- april spend 
...
(select 
	...
	sum(spending_amount) as total_spend
from spending_records
where month(spending_date) = 4  -- april spend 
group by customer_id)

select 
	cte1.customer_id,
	c.customer_name,
	cte1.total_spend as last_month_total_spend,
	cte2.total_spend as current_month_total_spend
from cte1 
join cte2 
