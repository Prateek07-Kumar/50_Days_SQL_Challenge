...
select 
	video_id,
	count(distinct(concat(user_firstname, user_lastname))) as count_user
from user_flags
...
SELECT 
    video_id,
	count(distinct flag_id) as unique_user
    --STRING_AGG(DISTINCT user_firstname, ', ') AS first_names,
    --STRING_AGG(DISTINCT user_lastname, ', ') AS last_names
FROM user_flags
WHERE flag_id IS NOT NULL
GROUP BY video_id;
