...
INSERT INTO customers3 (id, first_name, last_name, city, address, phone_number) VALUES
...
INSERT INTO orders6 (id, cust_id, order_date, order_details, total_order_cost) VALUES
...
SELECT 
    ROUND(
        ...
        CAST(COUNT(*) AS NUMERIC) * 100, 2
    ) AS shipable_orders_percentage
FROM orders6 AS o
LEFT JOIN customers3 AS c
    ON o.cust_id = c.id;
...
SELECT 
    ROUND(
        CAST(SUM(
            CASE
                WHEN c.phone_number is null or c.phone_number = '' THEN 1
                ELSE 0
            END
        ) AS NUMERIC) / 
        CAST(COUNT(*) AS NUMERIC) * 100, 2
