...
SELECT 
	city,
	avg(mkt_price) as avg_city_price
	from house_price
	group by city
), 
nation_avg as ( 
	select avg(mkt_price) as nation_avg_price
	from house_price
)
select 
	h1.id,
...
join city_avg cv on
h1.city = cv.city
cross join nation_avg nv
where h1.mkt_price > cv.avg_city_price
and h1.mkt_price < nv.nation_avg_price;
